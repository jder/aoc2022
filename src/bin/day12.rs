...
fn main() {
    ...
    let (start_row, start_col) = find('S').unwrap();
    let (end_row, end_col) = find('E').unwrap();
    ...
    let adjacent = |pos: (usize, usize)| {
        ...
        result
            .into_iter()
            .filter(move |(row, col)| height(map[*row][*col]) - height(map[pos.0][pos.1]) <= 1)
    };
    ...
    println!("Result: {}", distance[end_row][end_col])
}
