...
fn main() {
    ...
    let max_coord = *lines
        ...
        .unwrap();
    let max_coord = max(max_coord.0, max_coord.1) + 200; // "infinite"

    let max_y = lines
        .iter()
        .map(|line| line.iter().max_by_key(|(_, y)| y).unwrap().1)
        .max()
        .unwrap();
    ...
    paint_line(&mut filled, &[(0, max_y + 2), (max_coord, max_y + 2)]);
    ...
    'outer: for grain in 0.. {
        ...
        loop {
            if !filled[location.0 as usize][location.1 as usize + 1] {
                ...
                if location == (500, 0) {
                    println!("Grain {} plugged the opening", grain);
                    break 'outer;
                }
